...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.pushButton_5.clicked.connect(lambda: subprocess.Popen('onedrive --monitor', shell=True))       
    ...
    def startOneDriveMonitor(self, state):
        if state == 'start':
            self.onedrive_process = subprocess.Popen('onedrive --monitor', stdout=subprocess.PIPE, shell=True, preexec_fn=os.setsid)
        elif state == 'stop':
